...
def str_to_bool(val: Optional[str]) -> bool:
    """Converts common string representations of truth to True."""
...
def get_env_var(key: str, default: Any = None, required: bool = False, var_type: type = str) -> Any:
    ...
    value = os.getenv(key)
    if value is None:
        if required: raise ValueError(f"Missing required environment variable: {key}")
        return default

    original_value = value
    try:
        if var_type == bool: return str_to_bool(value)
        elif var_type == int: return int(value)
        elif var_type == float: return float(value)
        elif var_type == list: return parse_comma_sep_str(value) # Assumes comma-separated string
        elif var_type == dict: raise NotImplementedError("Direct dict parsing not implemented. Use JSON string.")
...
if _log_file:
    if "${TIMESTAMP}" in _log_file: # Simple timestamp replacement
...
TIME_SERIES_FREQUENCY: Optional[str] = get_env_var('TIME_SERIES_FREQUENCY', None)
_imputation_method = get_env_var('DATA_IMPUTATION_METHOD', 'none').lower()
valid_imputations = ['none', 'ffill', 'bfill', 'mean', 'median', 'interpolate']
if _imputation_method not in valid_imputations:
    logging.warning(f"Invalid DATA_IMPUTATION_METHOD '{_imputation_method}'. Using 'none'. Options: {valid_imputations}")
...
if not MODELS_TO_RUN: raise ValueError(f"No valid models specified in MODELS_TO_RUN. Check .env. Valid: {valid_models}")
...
SARIMA_SEASONAL_ORDER_NOS: Tuple[int, int, int] = (SARIMA_SP, SARIMA_SD, SARIMA_SQ) # S added dynamically
...
# Convert 'auto'/True/False string parameters
...
# KerasTuner Hyperparameter Ranges
...
# Validation Check: Tuner Objective vs Validation Size
if USE_KERAS_TUNER:
    ...
    if tuner_objective_needs_val and VALIDATION_SIZE <= NN_STEPS:
        raise ValueError(f"KerasTuner objective '{NN_TUNER_OBJECTIVE}' needs val data, but VALIDATION_SIZE ({VALIDATION_SIZE}) must be > NN_STEPS ({NN_STEPS}).")
    elif tuner_objective_needs_val and VALIDATION_SIZE == 0:
         raise ValueError(f"KerasTuner objective '{NN_TUNER_OBJECTIVE}' needs val data, but VALIDATION_SIZE is 0. Set VALIDATION_SIZE > NN_STEPS.")
...
valid_metrics = {'MAE', 'RMSE', 'MAPE'}
EVALUATION_METRICS = [m for m in EVALUATION_METRICS if m in valid_metrics]
if not EVALUATION_METRICS:
     logging.warning(f"No valid metrics in EVALUATION_METRICS. Using default: ['MAE', 'RMSE', 'MAPE']. Valid: {valid_metrics}")
...
if _forecast_horizon <= 0:
    logging.warning("FORECAST_HORIZON must be > 0. Setting to 12."); _forecast_horizon = 12
